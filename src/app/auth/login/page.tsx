...
export default function LoginPage() {
  ...
  return (
    <>
      <PageContainer title="Iniciar Sesión" fixWidth img={AuthImg}>
        <Stack component="form" spacing={2}>
          ...
          >
            {isLoading ? (
              <CircularProgress sx={{ color: palette.customColors.white }} />
            ) : (
              "Entrar"
            )}
          </Button>
      ...
      <LoadingComponent isLoading={isLoading} message="Iniciando sesión..." />
      <LoadingComponent
        status="error"
        isLoading={error?.data}
        message={`Error con la identificación ( ${error?.data.message} )`}
        redirect={error?.data?.needActive}
        redirectUrl="/auth/confirm_account"
      />
    </>
  );
}
